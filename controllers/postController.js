...
exports.getAllPosts = catchAsync(async (req, res, next) => {
    ...
    let posts = await Posts.find({createdBy: req.userId})
    .populate([{
        path: "createdBy",
    }, {
        path: "votes",
...
exports.deletePosts = catchAsync(async (req, res, next) => {
    ...
    if(!posts) {
        return next(new AppError(`No post found with id ${req.params.postId}`, 404));
    }
...
exports.updatePost = catchAsync(async (req, res, next) => {
    ...
    let modifiedPost = await Posts.findByIdAndUpdate(req.params.postId, {
        $set: data
    }, {
        new: true,
        runValidators: true,
